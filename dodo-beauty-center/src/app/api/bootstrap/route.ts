...
/**
 * One-time setup endpoint. Visit this URL once after deploying to create:
 * - the first admin user (from ADMIN_EMAIL / ADMIN_PASSWORD env vars)
 * - default settings, working hours, and demo services/offer
 *
 * Protected by NEXTAUTH_SECRET as a query param so randoms can't trigger it.
 * Safe to call more than once — it skips anything that already exists.
 *
 * Usage: https://yourdomain.com/api/bootstrap?key=YOUR_NEXTAUTH_SECRET
 */
export async function GET(req: NextRequest) {
  ...
  if (!existingAdmin) {
    ...
    results.push(`✅ تم إنشاء حساب الأدمن: ${email}`);
  } else {
    results.push(`ℹ️ حساب الأدمن موجود بالفعل: ${email}`);
  }
  ...
  results.push("✅ تم ضبط الإعدادات الافتراضية");

  // ---- Working hours (Sat–Thu open, Friday closed) ----
  const hours = [
  ...
  results.push("✅ تم ضبط أيام وساعات العمل");
  ...
  results.push("✅ تم إضافة الخدمات التجريبية");
